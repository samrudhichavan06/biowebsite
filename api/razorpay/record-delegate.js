...
export default async function handler(req, res) {
  ...
  try {
    ...
    const attendeeType = String(body.attendeeType || "Delegate").trim() || "Delegate";
    ...
    const receipt = String(body.receipt || paymentId || orderId || `rcpt_${Date.now()}`).trim();
    const registrationCode = String(body.registrationCode || `RCPT-${Date.now()}`).trim();

    if (!fullName || !email) {
      return sendJson(res, 400, { ok: false, error: "Missing delegate details" });
    }
    ...
    if (adminModule && adminModule.firestore) {
      ...
      await db.collection("delegates").doc(String(delegateDocId)).set(
        {
      ...
      await db.collection("registrations_bioenergy_global_2026").doc(String(delegateDocId)).set(
        {
          id: String(delegateDocId),
          ...recordBase,
    ...
    fs.appendFileSync(filePath, JSON.stringify({ ...recordBase, created_at: receivedAt, full_name: fullName, event_name: eventName, attendee_type: attendeeType }) + "\n");
